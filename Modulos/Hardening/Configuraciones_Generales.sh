...
disable_default_services(){
	...
	"console.*" "r*sync.*" "r*sys.*" "system.*" "d-bus.*")
	case $1 in
...
password_policy(){
	...
	case $1 in
		'Debian 9' | 'Debian 10')
			...
			log_errors $? "Instalación de libpam-cracklib"
			...
			sed -i "s/\(password.*sufficient.*\)pam_unix.*/\1pam_unix.so nullok use_authtok md5 shadow remember=5/" $PAM_D_COMMON_PASSWORD
			log_errors $? "Se restringe que los usuarios reutilicen alguna de sus últimas 5 contraseñas anteriores"
			sed -i "s/\(password.*requi.*\)pam_cracklib.*/\1pam_cracklib.so try_first_pass retry=3 minlen=8 lcredit=-1 ucredit=-1 dcredit=-1 ocredit=-1 difok=0 reject_username/" $PAM_D_COMMON_PASSWORD
			log_errors $? "Se aplica uso de contraseñas fuertes: longitud mínima de 8, con al menos 1 letra minúscula, \
			1 mayúscula, 1 dígito, 1 carácter especial"
			;;
		'CentOS 6' | 'CentOS 7')
			...
			sed -i "s/\(password.*sufficient.*\)pam_unix.*/\1pam_unix.so nullok use_authtok md5 shadow remember=5/" $PAM_D_CONF
			log_errors $? "Se restringe que los usuarios reutilicen alguna de sus últimas 5 contraseñas anteriores"
			sed -i "s/\(password.*requi.*\)$PAM_PASSW.*/\1$PAM_PASSW.so try_first_pass retry=3 minlen=8 lcredit=-1 ucredit=-1 dcredit=-1 ocredit=-1 difok=0 reject_username/" $PAM_D_CONF
			log_errors $? "Se aplica uso de contraseñas fuertes: longitud mínima de 8, con al menos 1 letra minúscula, \
			1 mayúscula, 1 dígito, 1 carácter especial"
			;;
	esac
	...
	log_errors $? "Expiración de contraseñas (nuevos usuarios): cada 90 días (se adevertirá 7 días antes) y mínimo 7"
	ACCOUNTS=$(grep "/bin/.*sh" /etc/passwd  | cut -d":" -f1)
	for ACCOUNT in $ACCOUNTS; do
		chage -M 90 -m 7 -W 7 $ACCOUNT
		log_errors $? "Expiración de contraseñas (usuarios existntes): cada 90 días (se adevertirá 7 días antes) y mínimo 7"
	done
}
