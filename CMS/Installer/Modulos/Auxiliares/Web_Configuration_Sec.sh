...
if [[ $WEB_SERVER == "Apache" ]]; then
	...
	echo '<FilesMatch "(?i)(^\.ht|README|robots|INSTALL|UP(D|GR)A(T|D)E|CHANGELOG|LICENSE|COPYING|CONTRIBUTING|TRADEMARK|EXAMPLE|PULL_REQUEST_TEMPLATE)(.*)$|(.*config|version|info|xmlrpc)(\.php)$|(.*\.(bak|conf|dist|fla|in[ci]|log|orig|sh|sql|t?(ar|ar\.gz|[^css]gz)|z(.*|ip)|~)$)">
			Require all denied
	...
	[[ $SO =~ Debian.* ]] && sed -i "s/\(\s*\)#\?\(\s\)\(server_tokens\s*\).*/\1\2\3off;/" $WEB_SERVER_CONF && log_errors $? "server_tokens off;"
	[[ $SO =~ CentOS.* ]] && sed -i '/http\s\+{/a 		server_tokens off;' $WEB_SERVER_CONF && log_errors $? "server_tokens off;"
	...
	[[ $SO =~ Debian.* ]] && sed -i '/^\s\+try_files $uri $uri\/ =404;/a 		autoindex off;' /etc/nginx/sites-available/default && log_errors $? "Se deshabilita listado de directorios"
	[[ $SO =~ CentOS.* ]] && sed -i '/^\s\+location\s\+\/\s\+{/a autoindex off;' $WEB_SERVER_CONF && log_errors $? "Se deshabilita listado de directorios"
	...
	[[ $SO =~ Debian.* ]] && echo 'proxy_cookie_path / "/; HTTPOnly; Secure";' >> /etc/nginx/sites-available/default && log_errors $? 'proxy_cookie_path / "/; HTTPOnly; Secure";'
	# Se aplica para todos
	...
	# Se aplica a todos
	[[ $SO =~ Debian.* ]] && sed -i '/^[^#]*server_name\s\+_;/a location ~* \/.*\(\(^\\.ht|README|robots|INSTALL|UP\(D|GR\)A\(T|D\)E|CHANGELOG|LICENSE|COPYING|CONTRIBUTING|TRADEMARK|EXAMPLE|PULL_REQUEST_TEMPLATE\)\(.*\)$|\(.*config|version|info|xmlrpc\)\(\\.php)$|\(.*\\.\(bak|conf|dist|fla|in[ci]|log|orig|sh|sql|t\(ar|ar\\.gz|[^css]gz\)|z\(.*|ip\)|~\)$\)\){\n\tdeny all;\n\terror_page 403 \/;\n}' /etc/nginx/sites-available/default && log_errors $? "Se restringe el acceso a los archivos publicos"
	[[ $SO =~ CentOS.* ]] && sed -i '/^[^#]*server_name\s\+_;/a location ~* \/.*\(\(^\\.ht|README|robots|INSTALL|UP\(D|GR\)A\(T|D\)E|CHANGELOG|LICENSE|COPYING|CONTRIBUTING|TRADEMARK|EXAMPLE|PULL_REQUEST_TEMPLATE\)\(.*\)$|\(.*config|version|info|xmlrpc\)\(\\.php)$|\(.*\\.\(bak|conf|dist|fla|in[ci]|log|orig|sh|sql|t\(ar|ar\\.gz|[^css]gz\)|z\(.*|ip\)|~\)$\)\){\n\tdeny all;\n\terror_page 403 \/;\n}' $WEB_SERVER_CONF && log_errors $? "Se restringe el acceso a los archivos publicos"
